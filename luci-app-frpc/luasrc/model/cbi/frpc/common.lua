...
-- Tab4: 服务端设置（从选中的 server section 读取）
local function serve_option(option, ...)
	local o = s:taboption("server", ...)
	o.cfgvalue = function(self, sec)
		local sn = self.map:get(sec, "server")
		if sn and sn ~= "" then
			return self.map.uci:get("frpc", sn, option) or ""
		end
		return ""
	end
	o.write = function(self, sec, val)
		local sn = self.map:get(sec, "server")
		if sn and sn ~= "" then
			self.map.uci:set("frpc", sn, option, val)
		end
	end
	return o
end

o = s:taboption("server", ListValue, "server", translate("服务端"))
o:value("", translate("无"))
for k, v in pairs(server_table) do
	o:value(k, v)
end
o.description = translate("以下字段直接作用于选中的服务端，无需跳转单独的服务器页面")

o = serve_option("serverAddr", Value, "serverAddr", translate("服务端地址"))
o.placeholder = "0.0.0.0"

o = serve_option("serverPort", Value, "serverPort", translate("服务端端口"))
o.datatype = "port"
o.placeholder = "7000"

o = serve_option("auth__method", ListValue, "auth__method", translate("鉴权方式"))
o:value("", translate("（空）"))
o:value("token")
o:value("oidc")

o = serve_option("auth__token", Value, "auth__token", translate("鉴权令牌"))
o.password = true

o = serve_option("transport__tcpMux", Flag, "transport__tcpMux", translate("关闭 TCP 复用"))
o.enabled = "false"
o.disabled = ""
o.description = translate("Frpc 默认开启 tcpMux")

o = serve_option("transport__tcpMuxKeepaliveInterval", Value, "transport__tcpMuxKeepaliveInterval", translate("tcpMux心跳检查间隔秒数"))
o.datatype = "uinteger"
o.placeholder = "30"
